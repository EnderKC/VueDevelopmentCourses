...
public class JwtUtil {
    ...
    * */
     ...
     * 生成邮箱验证码的token
     * */
    public static String createTokenEmail(String email) {
        try {
            // 设置过期时间（5分钟）
            Date date = new Date(System.currentTimeMillis() + EXPIRE_TIME_EMAIL);
            // 私钥和加密算法
            Algorithm algorithm = Algorithm.HMAC256(TOKEN_SECRET);
            // 设置头部信息
            Map<String, Object> header = new HashMap<>(2);
    ...
    * 从token中解析出email的信息
    * */
    public static String parseTokenEmail(String token){
        DecodedJWT jwt = JWT.decode(token);
