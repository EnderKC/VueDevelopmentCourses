  ...
  <div class="bodyStyle">
    ...
    <a-form ref="searchFormRef" name="advanced_search" class="ant-advanced-search-form formStyle" :model="searchFormState"
      ...
      <a-row>
        <a-col :span="12" style="text-align: left">
          <a-button type="primary" :icon="h(PlusOutlined)" v-if="userRole === 'admin'" @click="newStudent">
            新增
          </a-button>
        </a-col>
    ...
    <a-modal v-model:open="modalVisible" :title="dialogTitle" ok-text="确认" cancel-text="取消" width="500px" centered
      ...
      <a-form ref="dialogFormRef" :model="dialogFormState" :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-form-item has-feedback ref="id" label="ID" name="id" :rules="[
          { required: true, message: '请输入ID!' },
          { min: 1, max: 30, message: '长度在 3 to 30', trigger: 'blur' }
        ]" class="error-align">
          <a-input v-model:value="dialogFormState.id" v-if="dialogTitle !== '新增学生'" disabled />
          <a-input v-model:value="dialogFormState.id" v-else />
        </a-form-item>
        <a-form-item has-feedback ref="stuid" label="学号" name="stuid" :rules="[
          { required: true, message: '请输入学号!' },
          { min: 3, max: 30, message: '长度在 3 to 30', trigger: 'blur' }
        ]" class="error-align">
          <a-input v-model:value="dialogFormState.stuid" v-if="dialogTitle !== '新增学生'" disabled />
          <a-input v-model:value="dialogFormState.stuid" v-else />
        </a-form-item>
        <a-form-item has-feedback label="课程号" name="cid" :rules="[
          { required: true, message: '请输入课程号!' },
          { min: 1, max: 10, message: '长度在 1 to 10', trigger: 'blur' }
        ]" class="error-align">
          <a-input v-model:value="dialogFormState.cid" v-if="dialogTitle !== '新增学生'" disabled />
          <a-input v-model:value="dialogFormState.cid" v-else />
        </a-form-item>
        <a-form-item has-feedback label="成绩" name="score" class="error-align">
          <a-input v-model:value="dialogFormState.score" v-if="userRole !== 'admin'" disabled />
          <a-input v-model:value="dialogFormState.score" v-else />
        </a-form-item>
        <a-form-item ref="sdate" label="考试时间" name="sdate" class="error-align">
          <a-date-picker v-model:value="dialogFormState.sdate" />
        </a-form-item>
      </a-form>
    </a-modal>
    <!-------新增/更新对话框结束--------->
...
const columns: TableColumnsType = [
  ...
  { title: '学号', dataIndex: 'stuid', key: 'stuid', width: 100 },
  { title: '课程号', dataIndex: 'cid', key: 'cid', width: 100 },
  { title: '成绩', dataIndex: 'score', key: 'score', width: 100 },
  { title: '考试日期', dataIndex: 'sdate', key: 'sdate', width: 100 },
  {
    title: '操作',
    key: 'operation',
    fixed: 'right',
...
const update = (record) => {
  ...
  record.sdate = dayjs(record.sdate)
